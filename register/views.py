...
def _cart_id(request):
    cart2 = request.session.session_key
    if not cart2:
        cart2 = request.session.create()
    return cart2
...
def cart(request,total=0,quantity=0,cart_items=None):
    ...
    try:
        if request.user.is_authenticated:
            ...
            cart1 = Register.objects.get(register_id=_cart_id(request))
            cart_items = RegisterItem.objects.filter(register=cart1,is_active=True)
